...
async fn handle_messages(mutex: Mutex, pool: Pool<Postgres>) -> Result<(), async_nats::Error> {
    ...
    loop {
        ...
        let mut messages: Vec<nats::MarketHistories> = Vec::new();

        for message in queue_slice {
            let parse_result = serde_json::from_slice::<nats::MarketHistories>(&message);
        ...
        let rows_affected = match result {
            ...
            Err(err) => {
                print!(
                    "{} handle_messages: Failed to insert market histories: {}\n",
                    chrono::Local::now(),
                    err
                );
                continue;
        ...
        print!(
            "{} handle_messages: Inserted {} market histories in {} ms...\n",
            chrono::Local::now(),
            rows_affected,
